...
Getting Started
Rds/
create Rds in same vpc
Backend-setup/
create backend server and add i am role access to the s3 bucket
Clone the repository to your backend server.
...
Run the requirements.tx on backend path
pip install -r requirements.txt
change the rds details on rds.py
create database and tables by using below command
mysql -h rds-endpoint -u admin -p<rds-password> < paytm.sql
Finally run the rds.py
nohup python3 rds.py > rds.log 2>&1 &
S3-setup/
Create s3 bucket public or private if you create private you need to add bucket policy to
Frontend-setup/
create frontend server and add i am role access to the s3 bucket
Clone the repository to your frontend server.
sudo yum install git -y
git clone https://github.com/CloudTechDevOps/Paytm-fullstack-project.git
switch to frontend
 cd Paytm-fullstack-project/Frontend
siwtch to frontend s3
 cd Frontend-S3
install dependencys
sudo yum update -y
sudo yum install python3 -y
sudo yum install python3-pip -y
python3 --version pip3 --version
Run the requirements.tx on frontend-s3 path
pip install -r requirements.txt
change the s3 bucket name on backends3.py file if you are using private s3 add the bucket policy.Policy is there on s3backendpolicy.txt
...
Finally run the backends3.py
nohup python3 backends3.py > backend.log 2>&1 &
If you want to create loadbalncer for this s3 just create with 5000 port other wise ignore for pulic deployment

switch to frontend-code

cd ..
cd Frontend-code
change the backend url on following files signup.html signuppscannerhtml if your are using loadbalncer for backend paste loadbalncer url otherwise paste public ip:5000 like this change any one
// Set your backend API URL here
      const API_URL = "http://backend-1090030115.ap-northeast-1.elb.amazonaws.com";
    //const API_URL = "http://publicip-backend:5000";
then change the s3 url on summary.html file either loadbalncer or public:500 port
...
install the dependyces on frontend server
sudo yum update -y
sudo yum install httpd -y
sudo systemctl start httpd
sudo systemctl enable httpd
copy the frontend code to html path
sudo cp -r * /var/www/html
access the application with public ip or loadblancer url
sign up and login the appliaction
Usage
